...
const findUser = {
 ...
 byAccessToken: async function (accessToken) {
  try {
   console.log(
    `Searching for user with access token: ${
   ...
   const user = await User.findOne({
    "sensetive.accessToken.value": accessToken
   });

   // Log result
   if (user) {
    console.log(
     `User found by access token : ${
      user.username || user.gmail
     }`
    );
   } else {
    console.log("No user found with the provided access token");
    throw new Error("No user found with the provided access token");
   }
 ...
 byGmail: async function (gmail) {
  try {
   ...
   if (!user) {
    ...
    throw new Error(`No user found with gmail: ${email}`);
   }
 ...
 byProperty: async function (prop, value) {
  try {
   ...
   if (user) {
    console.log(
     `User found by ${prop}: ${user.username || user.gmail || user._id}`
    );
   } else {
    console.log(`No user found with ${prop} = ${value}`);
    throw new Error(`No user found with ${prop} = ${value}`);
   }
 ...
 byMultipleProperties: async function (queryObject) {
  try {
   ...
   if (user) {
    console.log(
     `User found with multiple properties: ${
      user.username || user.gmail || user._id
     }`
    );
   } else {
    console.log("No user found matching all specified properties");
    throw new Error("No user found matching all specified properties");
   }
...
throw new Error(`No user found with phone: ${cleanPhone}`);
   }
 ...
 byMatricNumber: async function (matno) {
  try {
   ...
   if (user) {
    ...
    throw new Error(`No user found with matric number: ${matno}`);
   }

   return user;
  } catch (error) {
   console.error(
    `Error finding user by matric number ${matno}:`,
    error.message
   );
   throw error;
  }
